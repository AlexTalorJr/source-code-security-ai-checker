...
- **Аутентификация через Bearer-токен** -- все API-эндпоинты (кроме /api/health) требуют валидный Bearer-токен в заголовке Authorization; токены генерируются для каждого пользователя через панель управления
